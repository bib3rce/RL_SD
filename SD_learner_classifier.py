...
from SDRule import *
from Beam_SD import *
from DoubleBeam_weights import *
from DoubleBeam import *
...
class SD_learner(orange.Learner):
    ...
    def_k = 4
    def_maxRules = 0
    def_weight_factor = 0.99
    def_refinement_heuristics = "Inverted Laplace"
    def_selection_heuristics = "Laplace"
    def_type = "harmonic"
    ...
    def __init__(self, db_type = def_type, name = def_name, algorithm = def_alg, weight_factor = def_weight_factor, minSupport = def_minSupport, \
                 minConfidence = def_minConfidence, beamWidth = def_beamWidth, g = def_g, \
                 k = def_k, max_rules = def_maxRules, refinement_heuristics=def_refinement_heuristics,\
                 selection_heuristics = def_selection_heuristics):

        #print minSupport
        # parameter checking
        if algorithm not in ["SD","Apriori-SD","CN2-SD", "DoubleBeam", "DoubleBeam-weights"]:
        ...
        if type(weight_factor) not in [float]:
            #or weight_factor < 0.0 or weight_factor > 1.0:
            raise ValueError('The geometric weight factor should be float in the [0.0, 1.0] range.')
        ...
        elif algorithm == "DoubleBeam":
            self.learner = DoubleBeam(minSupport, beamWidth, g, refinement_heuristics, selection_heuristics)
        elif algorithm == "Apriori-SD":
            self.learner = Apriori_SD(minSupport, minConfidence, k)
        elif algorithm == "CN2-SD":
            self.learner = CN2_SD( k )
        elif algorithm == "DoubleBeam-weights":
            self.learner = DoubleBeam_weights(type = db_type,weight_factor=weight_factor,minSupport=minSupport, beam_width=beamWidth, g=g, refinement_heuristics=refinement_heuristics, selection_heuristics=selection_heuristics)
        else:
