...
#                        if len(d_attribute.getValueFrom.transformer.points) > 0:
        ...
        while (tmpRule.quality>0) and (self.max_rules==0 or len(rules)<self.max_rules):
            ...
            #if bestRule.orderedRuleToString() not in added_rules:
            rules.append(bestRule)
            #added_rules.append(bestRule.orderedRuleToString())
            tmpRule =  self.rbf(data, self.weightID, targetClass, None)
        ...
        if len(rules)==0:
            rules = [SDRule(data=data, targetClass=targetClass)]
...
if __name__=="__main__":
    ...
    rules = learner(data, targetClass=targetClass, num_of_rules=2)
    rules.printRules()